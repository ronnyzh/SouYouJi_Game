    ...
    export namespace fla {
        export class TipsObj {
            ...
            getCardNumber(cardId) {
                return fla.utils.getCardNumber(cardId);
            }
            ...
            getSingleCardList(count, excludeList): Array<any> {
            ...
            // hasRocket()
            // {
            //     return this.cardIdList.indexOf(fla.CONSTANTS.RED_JOKER_ID) != -1 && this.cardIdList.indexOf(fla.CONSTANTS.BLACK_JOKER_ID) != -1;
            // }
            //
            // addRocket(list)
            // {
        ...
        export class SingleCardTips extends TipsObj {
            getTipsList() {
                ...
                for (var i = r1List.length - 1; i >= 0; --i) {
                    var id = r1List[i];
                    if (fla.utils.greaterThan(this.opponent.getKeyValue(), id))
                        tipsList.push([id]);
                }

                this.iterRepeatList(2, 4, function (count, list) {
                    if (fla.utils.greaterThan(this.opponent.getKeyValue(), list[0]))
                ...
                this.iterRepeatList(3, 4, function (count, list) {
                    ...
                    switch (this.opponent.getType()) {
                        ...
                        case fla.CARD_TYPE.TRIPLET_WITH_TWO:
                            var tmp = this.getPairList(1, list);
                            if (tmp != null && tmp.length > 0)
                                tipsList.push(list.slice(count - 3).concat(tmp[0]));
                            break;
                    }
                }.bind(this));
        ...
        export class TripletSequenceTips extends TipsObj {
            getTipsList() {
                ...
                var lenOpponentTriplet = opponentList.length / itemCount;
        ...
        export class QuadplexSetTips extends TipsObj {
            getTipsList() {
                ...
                if (this.opponent.getType() != fla.CARD_TYPE.BOMB)
                    this.addBomb(tipsList);
                return tipsList;
            }
        }
            ...
            getTipsObj(opponent, cardIdList) {
                ...
                switch (opponent.getType()) {
                    case fla.CARD_TYPE.SEQUENCE:
                        obj = new fla.SequenceTips(opponent, cardIdList);
                        break;

                    case fla.CARD_TYPE.SINGLE_CARD:
                        obj = new fla.SingleCardTips(opponent, cardIdList);
                        break;

                    case fla.CARD_TYPE.PAIR:
                        obj = new fla.PairTips(opponent, cardIdList);
                        break;

                    case fla.CARD_TYPE.TRIPLET:
                    case fla.CARD_TYPE.TRIPLET_WITH_ONE:
                    case fla.CARD_TYPE.TRIPLET_WITH_TWO:
                        obj = new fla.TripletTips(opponent, cardIdList);
                        break;

                    case fla.CARD_TYPE.BOMB:
                    case fla.CARD_TYPE.QUADPLEX_SET_WITH_ONE:
                    case fla.CARD_TYPE.QUADPLEX_SET_WITH_TWO:
                        obj = new fla.QuadplexSetTips(opponent, cardIdList);
                        break;

                    case fla.CARD_TYPE.SEQUENCE_OF_PAIRS:
                        obj = new fla.PairSequenceTips(opponent, cardIdList);
                        break;

                    case fla.CARD_TYPE.SEQUENCE_OF_TRIPLETS:
                    case fla.CARD_TYPE.SEQUENCE_OF_TRIPLETS_WITH_ONE:
                    case fla.CARD_TYPE.SEQUENCE_OF_TRIPLETS_WITH_TWO:
                        obj = new fla.TripletSequenceTips(opponent, cardIdList);
                        break;
