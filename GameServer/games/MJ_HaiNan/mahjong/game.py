...
class Game(PublicGame):
    ...
    def onDiscard(self, player, tile):
        ...
        # if player.handleMgr.isFirstDiscard == None:
        # player.handleMgr.isFirstDiscard = True

        # elif player.handleMgr.isFirstDiscard == True:
        # player.handleMgr.isFirstDiscard = False

        # lastDiscardSide = (self.dealer.chair + 3) % self.maxPlayerCount

        # if player.chair == lastDiscardSide \
        #                     and player.handleMgr.isFirstDiscard:
        #     firstDiscard = ''
        #     self.dealer.isKeptDiscard = True

        #     for other in self.getPlayers():
        #         if not firstDiscard:
        #             firstDiscard = other.handleMgr.getDiscardTiles()[0]
        #             continue

        #         if other.handleMgr.getDiscardTiles()[0] != firstDiscard:
        #             self.dealer.isKeptDiscard = False
        #             break

        # if self.dealer.isKeptDiscard:
        # 发送手牌被跟协议
        # resp = hainan_mahjong_pb2.S_C_Be_Follow()
        # resp.side = self.dealer.side
        # self.sendAll(resp)
