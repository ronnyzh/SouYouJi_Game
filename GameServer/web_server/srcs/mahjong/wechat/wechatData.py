...
# APPID_2 = 'wx4a6ff1665852a35e'
# SECRET_2 = '321461fa5bf96a7fe7d13ae6751dec97'
APPID = 'wxa3cb327cd529862d'
SECRET = '1e3554f8dca4ec2414e303e5ed6c5148'
APPID_2 = 'wxa3cb327cd529862d'
SECRET_2 = '1e3554f8dca4ec2414e303e5ed6c5148'
...
MCH_KEY_TX = '1e47b5f0ce90039a6f8c61f4c8bd8a25'
MCH_ID_FISH = '1495588962'
MCH_KEY_FISH = '974a0bbae55f0dc9bb2270cbb63679b5'
NOTIFY_URL_FISH = 'http://%s/fish/notifyServer'#通知地址^
APPIDMINI = 'wxa7286b32c98ba234'
SECRETMINI = 'ab3a92de064e43fe25615dca2c216e7a'
...
def checkWeixinCode(code):#校验微信code
    ...
    print('[checkWeixinCode] %s'%(message))
...
def checkWeixinCode_2(code):#校验微信code
    print '----------------------------------------------------'
    print '校验微信 appid 2'
    print 'appid_2'
    print APPID_2
    print '----------------------------------------------------'
    url = "https://api.weixin.qq.com/sns/oauth2/access_token?appid=%s&secret=%s&code=%s&grant_type=authorization_code"%(APPID_2, SECRET_2, code)
    message = getUrlMessage(url)
    print '----------------------------------------------------'
    print 'message'
    print message
...
def checkWeixinCodeWEB(code):#微信公众号校验微信code
    ...
    if 'errcode' in message:
        ...
        return message
def checkWeixinCodeMINI(code):#微信公众号校验微信code
    url = "https://api.weixin.qq.com/sns/oauth2/access_token?appid=%s&secret=%s&code=%s&grant_type=authorization_code"%(APPIDMINI, SECRETMINI, code)
    message = getUrlMessage(url)
    if 'errcode' in message:
        log_util.error('weixin check code error:[%s]'%message)
        return False
...
def setOpenid2account(openID, accessToken, refreshToken, ip, redis, code):#微信号绑定账号
    ...
    from mahjong.model.goldModel import player_add_gold
    player_add_gold(redis, account, 0)
